...
export class BlogsQueryRepository {
    ...
    async findAll({
        ...
        const blogs = await this.blogModel.find({name: {$regex: searchNameTerm, $options: "(?i)a(?-i)cme"}})
            .sort([[sortBy, sortDirection]]).skip(getSkipNumber(pageNumber, pageSize)).limit(pageSize).lean()
        const totalCountBlogs = await this.blogModel.find({
            name: {
    ...
    async findOne(id: string): Promise<CreateBlogDbType | null> {
        ...
        if (findBlog) {
            const blog = new CreateBlogDbType(findBlog.id, findBlog.name, findBlog.youtubeUrl, findBlog.createdAt)
            return blog
        }
        return findBlog
    }
